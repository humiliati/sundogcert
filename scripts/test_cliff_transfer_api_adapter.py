...
def _openai_opener(captured):
    def fake_open(req, timeout):
        ...
        class _R:
            ...
            def read(self_):
                return json.dumps({
                    "choices": [{"message": {"content": "ACCEPT stale contradiction"}}],
...
def _anthropic_opener():
    def fake_open(req, timeout):
        class _R:
            ...
            def read(self_):
                return json.dumps({
                    "content": [{"type": "text", "text": "REJECT keep boundary"}],
...
def test_chat_once_openai_compatible_parses_and_sets_user_agent():
    ...
    assert s.text == "ACCEPT stale contradiction"
...
def test_chat_once_anthropic_parses():
    ...
    assert s.text == "REJECT keep boundary"
...
def test_generate_returns_k_samples_and_counts_tokens():
    ...
    assert out.draft.startswith("ACCEPT")
...
def test_sample_retries_on_transient_timeout(monkeypatch):
    ...
    assert out.draft.startswith("REJECT")  # recovered after the timeout
